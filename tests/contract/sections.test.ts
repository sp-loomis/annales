...
describe("POST /entries/:entryId/sections", () => {
  it("creates an empty section with increasing order", async () => {
    ...
    const first = await api(app, "POST", `/entries/${entryId}/sections`, {});
    expect(first.status).toBe(201);
    expect(first.body).toMatchObject({ entryId, order: 1, contentJson: null });
  ...
  it("404s on an unknown entry", async () => {
    const res = await api(
      app,
      "POST",
      "/entries/00000000-0000-0000-0000-000000000000/sections",
      {}
    );
...
describe("PATCH /sections/:id", () => {
  ...
  it("updates order without touching content", async () => {
    const { entryId } = await ctx();
    const created = await api(app, "POST", `/entries/${entryId}/sections`, {});
    const res = await api(app, "PATCH", `/sections/${created.body.id}`, { order: 5 });
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ order: 5, contentJson: null });
  });
});
